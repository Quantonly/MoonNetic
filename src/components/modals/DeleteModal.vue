    ...
    <div class="relative p-4 w-full max-w-xl h-full md:h-auto">
      <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
        ...
        <div class="p-6 space-y-6">
          ...
          <ul class="ml-10">
            <li v-for="file in fileNames" :key="file" class="text-red-500">
              - {{ file }}
            </li>
          </ul>
        </div>
        <div class="flex items-center p-6 space-x-2 rounded-b border-t border-gray-200 dark:border-gray-600">
          <button @click="deleteFiles()" type="button" class="text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800">Delete</button>
...
export default {
  ...
  methods: {
    async deleteFiles () {
      await FileService.deleteFiles({ fileNames: this.fileNames }).then((response) => {
        this.$emit('refresh')
      })
    },
    close () {
